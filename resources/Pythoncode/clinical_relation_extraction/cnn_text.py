...
class CnnRelation(object):

    def __init__(self, num_classes, seq_len, word_dict_size, pos_dict_size, chunk_dict_size, d1_dict_size, d2_dict_size,
                 type_dict_size, wv, batch_size=50, w_emb_size=50, d1_emb_size=5, d2_emb_size=5, pos_emb_size=5,
                 chunk_emb_size=5, dep_emb_size=5, type_emb_size=5, filter_sizes=None, num_filters=100,
                 l2_reg_lambda=0.0):
        if filter_sizes is None:
            filter_sizes = [4, 6]
        self.dep_emb_size = dep_emb_size
    ...
    def train_step(self, W_batch, pos_batch, chunk_batch, d1_batch, d2_batch, t_batch, y_batch):
        ...
        _, step, loss, accuracy, predictions = self.sess.run(
            [self.train_op, self.global_step, self.loss,
             self.accuracy, self.predictions], feed_dict)
    ...
    def test_step(self, w_batch, pos_batch, chunk_batch, d1_batch, d2_batch, t_batch, y_batch):
        ...
        _, step, loss, accuracy, predictions = self.sess.run([self.global_step, self.loss, self.accuracy,
